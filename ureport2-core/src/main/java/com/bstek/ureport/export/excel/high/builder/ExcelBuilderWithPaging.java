...
public class ExcelBuilderWithPaging extends ExcelBuilder{
	public void build(Report report, OutputStream outputStream,boolean withSheet) {
		...
		//modify by cooper 2017/10/12 09:32 start
		...
		try{
			...
			for(Page page:pages){
				if(withSheet){
					if(!pagingMode.equals(PagingMode.datePage)){
						sheetName = "第"+pageIndex+"页";
					}else{
						sheetName = page.getSheetName();
					}
					sheet=createSheet(wb, paper, sheetName);
					//modify by cooper 2017/10/12 09:32 end
					rowNumber=0;
				}else if(sheet==null){
